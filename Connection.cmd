...
:: --- File Connection_URLs ---
set Connection_URL='https://raw.githubusercontent.com/Moh-Dabas/Online-install-of-all-essential-windows-softwares/refs/heads/main/Connection.ps1'
...
set Connection_FILE='%TEMP%\Connection.ps1'
set PSM1_FILE='%TEMP%\AllFunctions.psm1'
set "Connection=!Connection_FILE:~1!"
set "Connection=!Connection:~0,-1!"
set "PSM1=!PSM1_FILE:~1!"
set "PSM1=!PSM1:~0,-1!"

Echo Deleting old files...
del /f /q "%Connection%" >nul 2>&1
...
echo Downloading %Connection% from %Connection_URL% using Bits (attempt !RETRIES!) >> "%LOG%"
powershell -NoProfile -ExecutionPolicy Bypass -nologo -Command "Start-BitsTransfer -Source %Connection_URL% -Destination %Connection_FILE%" >> "%LOG%" 2>>&1
...
:IWR
echo Downloading %Connection% from %Connection_URL% using IWR (attempt !RETRIES!) >> "%LOG%"
powershell -NoProfile -ExecutionPolicy Bypass -nologo -Command "Invoke-WebRequest -Uri %Connection_URL% -OutFile %Connection_FILE%" >> "%LOG%" 2>>&1
...
:: --- Run the main script ---
echo Running Script ...
echo Running %Connection%... >> "%LOG%"
set "CALLER_SCRIPT=%~f0"
powershell -NoProfile -ExecutionPolicy Bypass -nologo -File "%Connection%" "%CALLER_SCRIPT%"
if not "%errorlevel%"=="0" (
    echo  ERROR: %Connection% execution failed. >> "%LOG%"
    goto :exit
)
...
if not exist "%Connection%" (
    echo ERROR: %Connection% missing after download.
    echo  ERROR: %Connection% missing. >> "%LOG%"
    Set Verify=Failed
)
if not exist "%PSM1%" (
